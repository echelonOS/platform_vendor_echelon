...
func init() {
	android.RegisterModuleType("echelon_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = echelonExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = echelonExpandVariables(ctx, dep_file)
	...
	cmd := echelonExpandVariables(ctx, String(g.properties.Cmd))
